...
fn main() -> Result<(), std::io::Error> {
    ...
    let part1 = values
        .iter()
        .map(|v| extrapolate_for_part1(&v))
        .sum::<i64>();
    dbg!(part1);

    let part2 = values
        .iter()
        .map(|v| extrapolate_for_part2(&v))
        .sum::<i64>();
    dbg!(part2);
...
fn extrapolate_for_part1(history: &Vec<i64>) -> i64 {
...
fn extrapolate_for_part2(history: &Vec<i64>) -> i64 {
    let mut histories = vec![history.clone()];
    let mut current = history.clone();
    while !current.iter().all(|&v| v == 0) {
        let next = find_next(&current);
    ...
    for i in 0..histories.len() {
        let index = histories.len() - i - 1;
        value = histories[index].first().unwrap() - value;
    }
